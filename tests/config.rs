...
fn default_config_matches_documented_values() {
    ...
    assert_eq!(config.line_breaks.max_line_length, 80);
...
#[test]
fn invalid_json_and_invalid_enum_return_errors() {
    let dir = tempdir().unwrap();
...
#[test]
fn load_from_base_handles_relative_explicit_paths_and_missing_files() {
    let dir = tempdir().unwrap();
    ...
    let loaded = load_from_base(dir.path(), Some(std::path::Path::new("missing.json"))).unwrap();
