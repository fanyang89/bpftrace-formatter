...
type ASTFormatter struct {
	...
	// Use bytes.Buffer because its Reset() method preserves underlying capacity
	// for reuse, unlike strings.Builder.Reset().
	output         bytes.Buffer
	indentLevel    int
	lastWasNewline bool
	needIndent     bool
	...
	// Persistent visitor to avoid allocations on every FormatTree call
	visitor *ASTVisitor
}
